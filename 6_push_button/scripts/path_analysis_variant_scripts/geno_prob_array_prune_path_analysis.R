...
for (i in names(Genotypes_copy)) {
  ...
  test_marker <- as.vector(Genotypes_copy[[i]][, 1])
  ...
  cor_vector <- unlist(mclapply(1:length(Genotypes_subset), function(x) cor(test_marker, as.vector(Genotypes_subset[[x]][, 1])), mc.cores = max_cores))
  ...
  if (any(abs(cor_vector) >= cor_threshold, na.rm = TRUE)) {
    ...
    high_cor_markers <- names(Genotypes_subset)[which(abs(cor_vector) >= cor_threshold)]
    high_cor_markers_pos <- which(names(Genotypes_copy) %in% high_cor_markers)
    bad_markers <- c(bad_markers, high_cor_markers)
    Genotypes_copy <- Genotypes_copy[-high_cor_markers_pos]
  }
}
