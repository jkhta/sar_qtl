...
GridLMM_scan <- function(pheno, pheno_name, geno, map, proximal, sig_qtl) {
  ...
  if (pheno_name == "r_dry_gxe") {
    pheno_cov <- "bd_geno + bd_gxe"
  } else if (pheno_name == "h3_h1_gxe") {
    pheno_cov <- "bd_geno + bd_gxe + r_dry_geno + r_dry_gxe"
  } else if (pheno_name == "i_dry_gxe") {
    pheno_cov <- "bd_geno + bd_gxe + r_dry_geno + r_dry_gxe + h3_h1_geno + h3_h1_gxe"
  }
  
  if (pheno_name %in% c("r_dry_gxe", "h3_h1_gxe", "i_dry_gxe")) {
    if (length(sig_qtl) < 1) {
      model_formula <- as.formula(paste(pheno_name, " ~ ", pheno_cov, " + (1|geno)", sep = ""))
      ...
      model_formula <- as.formula(paste(pheno_name, " ~ ", pheno_cov, " + ", sig_qtl_formula, "+ (1|geno)", sep = ""))
    }
  } else {
    if (length(sig_qtl) < 1) {
...
GridLMM_stepwise <- function(list_of_pop_things, pheno_name, threshold, max_markers) {
  ...
  if (pheno_name %in% c("r_dry_gxe", "h3_h1_gxe", "i_dry_gxe")) {
    beta_ind <- tail(1:ncol(ind_pop_betas), length(sig_qtl))
    ind_pop_snp_betas <- ind_pop_betas[, ..beta_ind]
  } else {
...
if (perm_type == "geno") {
  ...
  pheno_perms_list <- pheno_perms_list[grepl("geno_path_analysis_perms", pheno_perms_list)]
...
pheno_stepwise_file_name <- paste(ind_pheno, pheno_type, "cov_GridLMM_stepwise_model.RDS", sep = "_")
snp_info_file_name <- paste(ind_pheno, "cov_qtl_info.csv", sep = "_")
