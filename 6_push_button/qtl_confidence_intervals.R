...
if (pheno_type == "univariate") {
  pheno_sw_model_file <- pheno_sw_model_files[grepl("univar", pheno_sw_model_files)]
  test_model <- readRDS(pheno_sw_model_file)
} else if (pheno_type == "multivariate") {
  pheno_sw_model_file <- pheno_sw_model_files[grepl("geno_GridLMM", pheno_sw_model_files)]
  test_model <- readRDS(pheno_sw_model_file)
}
