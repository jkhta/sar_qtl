...
setwd("/Users/jkhta/Documents/GitHub/sar_qtl/genotype_validation/genotype/")
...
past_genotypes_FRI_dist <- unlist(lapply(past_genotypes_chr_4, function(x) subset(x, cM_diff == min(cM_diff))$cM_diff))
past_genotypes_FRI_dist <- unlist(lapply(past_genotypes_chr_4, function(x) subset(x, Mb_diff == min(Mb_diff))$Mb_diff))
...
for (i in 1:length(past_genotypes_FRI)) {
    ...
    FRI_merged[FRI_merged == "NA"] <- NA
    FRI_merged[, 2] <- factor(FRI_merged[, 2], levels = c("A", "B"))
    FRI_merged[, 3] <- factor(FRI_merged[, 3], levels = c("A", "B"))
...
#reading out the correlation information between Marc's genotypes and the publicly available genotypes
FRI_correlation$dist <- past_genotypes_FRI_dist
FRI_correlation

#now working on the chromosome 5 genotypes
...
for (i in 1:length(past_genotypes_chr_5)) {
    past_genotypes_chr_5[[i]]$cM_diff <- abs(past_genotypes_chr_5[[i]]$CR_cM - marker_anno_FRI_FLC$cM[2])
    past_genotypes_chr_5[[i]]$Mb_diff <- abs(abs(((past_genotypes_chr_5[[i]]$Start_Mb * 1000000) - max(past_genotypes_chr_5[[i]]$Start_Mb * 1000000)))
 - marker_anno_FRI_FLC$Start_Mb[2])
}
...
past_genotypes_FLC_dist <- unlist(lapply(past_genotypes_chr_5, function(x) subset(x, cM_diff == min(cM_diff))$cM_diff))
past_genotypes_FLC_dist <- unlist(lapply(past_genotypes_chr_5, function(x) subset(x, Mb_diff == min(Mb_diff))$Mb_diff))
...
for (i in 1:length(past_genotypes_FLC)) {
    ...
    FLC_merged <- merge(geno_array_FLC, past_genotypes_FLC[[i]], by = "geno")
    FLC_merged[FLC_merged == "NA"] <- NA
    FLC_merged[, 2] <- factor(FLC_merged[, 2], levels = c("A", "B"))
    FLC_merged[, 3] <- factor(FLC_merged[, 3], levels = c("A", "B"))
...
FLC_correlation$dist <- past_genotypes_FLC_dist
FLC_correlation
