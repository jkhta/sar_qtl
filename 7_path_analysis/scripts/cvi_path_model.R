...
m_best <- "
bd ~ c(a, A) * AB + c(b, B) * BB
rdry ~ c(c, C) * bd + c(d, D) * AB
h3h1 ~ c(e, E) * bd + c(f, F) * rdry + c(g, G) * AB + c(h, H) * BB
idry ~ c(i, I) * bd + c(j, J) * rdry + c(k, K) * h3h1 + c(l, L) * AB
...
BB_idry_ind_sun := (b * i) + (b * c * j) + (b * e * k) + (b * c * f * k) + (h * k)
BB_idry_ind_shade := (B * I) + (B * C * J) + (B * E * K) + (B * C * F * K) + (H * K)
BB_idry_ind_diff := ((B * I) + (B * C * J) + (B * E * K) + (B * C * F * K) + (H * K)) - ((b * i) + (b * c * j) + (b * e * k) + (b * c * f * k) + (h * k))
"
...
setwd("/Users/jkhta/Desktop/nam_cam_fixing/34 - pop_path_analysis/output/")
fwrite(path_parameters_eff, "cvi_path_eff.csv", sep = ",", row.names = FALSE)
