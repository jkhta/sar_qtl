...
h3h1 ~ c(f, F) * B5
idry ~ c(g, G) * rdry + c(h, H) * h3h1 + c(i, I) * B4
...
B4_idry_ind_sun := a * d * g
B4_idry_ind_shade := A * D * G
B4_idry_ind_diff := (A * D * G) - (a * d * g)
...
B42_idry_ind_sun := b * d * g
B42_idry_ind_shade := B * D * G
B42_idry_ind_diff := (B * D * G) - (b * d * g)
...
B5_idry_ind_sun := (c * d * g) + (e * g) + (f * h)
B5_idry_ind_shade := (C * D * G) + (E * G) + (F * H)
