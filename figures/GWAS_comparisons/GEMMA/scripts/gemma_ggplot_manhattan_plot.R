...
setwd("/Users/jkhta/Documents/GitHub/sar_qtl/figures/GWAS_comparisons/GEMMA/output/")
...
ggplot(gemma_bd_geno_lm_ouput_change, aes(x = PScum, y = -log10(p_lrt))) + 
    ...
    xlab("Chromosome") + 
    ylab("-log10(p)")
setwd("/Users/James/Documents/GitHub/sar_qtl/figures/GWAS_comparisons/GEMMA/img/")
ggsave("gemma_bd_geno_lm_manhattan_plot.png", device = "png", width = 10, height = 8)

#the results with the kinship matrix
setwd("/Users/James/Documents/GitHub/sar_qtl/figures/GWAS_comparisons/GEMMA/output/")
...
ggplot(gemma_bd_gxe_ouput_change, aes(x = PScum, y = -log10(p_lrt))) + 
    ...
    scale_color_npg() +
    #facet_grid(~ chr, scales = "free") +
    scale_x_continuous(label = gemma_bd_gxe_breaks$chr, breaks = gemma_bd_gxe_breaks$center) +
    theme(panel.spacing = unit(0, "lines")) +
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
          panel.background = element_blank(), axis.line = element_line(colour = "black")) +
    theme(legend.position = "none") +
    ...
    xlab("Chromosome") + 
    ylab("-log10(p)")
setwd("/Users/jkhta/Documents/GitHub/sar_qtl/figures/GWAS_comparisons/GEMMA/img/")
ggsave("gemma_bd_gxe_lmm_manhattan_plot.png", device = "png", width = 10, height = 8)

#bd_gxe results with kinship matrix
setwd("/Users/jkhta/Documents/GitHub/sar_qtl/figures/GWAS_comparisons/GEMMA/output/")
gemma_bd_gxe_with_covar_ouput <- fread("nam_bd_gxe_lmm_with_covar.assoc.txt", sep = "\t", header = TRUE, stringsAsFactors = FALSE)
gemma_bd_gxe_with_covar_ouput$chr <- with(gemma_bd_gxe_with_covar_ouput, as.integer(sapply(strsplit(rs, split = "_"), function(x) x[1]))) 
gemma_bd_gxe_with_covar_ouput$ps <- with(gemma_bd_gxe_with_covar_ouput, as.integer(sapply(strsplit(rs, split = "_"), function(x) x[2]))) 
head(gemma_bd_gxe_with_covar_ouput)

gemma_bd_gxe_with_covar_ouput_change <- gemma_bd_gxe_with_covar_ouput %>% 
    ...
    # Add this info to the initial dataset
    left_join(gemma_bd_gxe_with_covar_ouput, ., by=c("chr"="chr")) %>%
    
    # Add a cumulative position of each SNP
    arrange(chr, ps) %>%
    mutate( PScum=ps+tot)

gemma_bd_gxe_with_covar_breaks <- gemma_bd_gxe_with_covar_ouput_change %>% group_by(chr) %>% summarize(center = (max(PScum) + min(PScum)) / 2)
gemma_bd_gxe_with_covar_breaks$chr <- as.integer(gemma_bd_gxe_with_covar_breaks$chr)
gemma_bd_gxe_with_covar_breaks$center <- as.numeric(gemma_bd_gxe_with_covar_breaks$center)

ggplot(gemma_bd_gxe_with_covar_ouput_change, aes(x = PScum, y = -log10(p_lrt))) + 
    geom_point(aes(color = as.factor(chr))) +
    scale_color_npg() +
    #facet_grid(~ chr, scales = "free") +
    scale_x_continuous(label = gemma_bd_gxe_with_covar_breaks$chr, breaks = gemma_bd_gxe_with_covar_breaks$center) +
    theme(panel.spacing = unit(0, "lines")) +
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
          panel.background = element_blank(), axis.line = element_line(colour = "black")) +
    theme(legend.position = "none") +
    ...
    ylab("-log10(p)")
setwd("/Users/jkhta/Documents/GitHub/sar_qtl/figures/GWAS_comparisons/GEMMA/img/")
ggsave("gemma_bd_gxe_with_covar_lmm_manhattan_plot.png", device = "png", width = 10, height = 8)
