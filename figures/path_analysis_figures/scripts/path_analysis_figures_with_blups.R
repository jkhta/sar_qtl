...
path_data_diff_B52 <- subset(path_data_FRI_FLC_ind, env == "diff" & allele_comb == "B52")
path_data_diff_B52[path_data_diff_B52 == 0] <- NA
path_data_diff_B52_ggplot <- ggplot(data = path_data_diff_B52, aes(x = est, y = pop_facet, color = effect_type)) +
    geom_point(size = 4) +
    geom_errorbarh(aes(xmax = est + se, xmin = est - se), height = 0.25, size = 2) +
    geom_vline(xintercept = 0, 
               color = "black", size = 0.25) + 
